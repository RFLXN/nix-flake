...
in
{
  ...
  assertions = [
    {
      assertion = monitors == null || (lib.isList monitors && lib.all lib.isAttrs monitors);
      message = "hyprland.useHyprland: monitors must be a list of hl.monitor spec attrsets.";
    }
    {
      assertion = workspaces == null || (lib.isList workspaces && lib.all lib.isAttrs workspaces);
